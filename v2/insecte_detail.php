			...
			<table width="100%" border="0" cellspacing="1" cellpadding="1">
              <tr>
                <td><table width="100%" border="0" cellspacing="1" cellpadding="1">
                  <tr>
					<? if (strlen($rec['photo_insecte'])>0) { ?>				
                    <td width="240" align="center" valign="top">
					<?
					if (strlen($rec['photo_insecte'])>0) {					
					$t1 = getimagesize("images/insecte/".$rec['photo_insecte']);
					?>
					<img src="images/insecte/<? echo $rec['photo_insecte']?>" width="200" onclick="MM_openBrWindow('photozoom.php?fn=<? echo "images/insecte/".$rec['photo_insecte']?>','','width=<? echo $t1[0]; ?>,height=<? echo $t1[1]; ?>')">
					<? }?>
					</td>
					<? }?>
                    <td valign="top">
					<table width="100%" border="0" cellpadding="1" cellspacing="1" class="texttitre">
					<? if (strlen($rec['nom_insecte']) > 0) { ?>
                      <tr>
                        <td align="center"><b><? echo ucwords($rec['nom_insecte']); ?></b></td>
                      </tr>
					<? } ?>
                    </table>
					<table width="100%" border="0" cellspacing="1" cellpadding="1">
                      <? if (strlen($rec['texte_insecte']) > 0) { ?>
                      <tr>
                        <td align="left"><br />
                            <b>G&eacute;n&eacute;ralit&eacute;s :</b><br />
                          <? echo $rec['texte_insecte']; ?></td>
                      </tr>
                      <? } ?>
                    </table></td>
                  </tr>
...
<map name="Map" id="Map"><area shape="rect" coords="75,33,133,97" href="http://arpcv.free.fr" target="_blank" />
</map></body>
</html>