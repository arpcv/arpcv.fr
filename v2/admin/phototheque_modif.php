<?
	...
	if (isset($_REQUEST['id'])) {
		$sql = "select * from photo where id_photo =".$_REQUEST['id'];
		$qry = mysql_query($sql);
	...
	if (isset($_POST['post'])) {
		...
		$sql = "update photo set nom_photo = '".$_POST['nom']."', idrubrique_photo = '".$_POST['rubrique']."', photo_photo = '".$photo."' where id_photo = ".$_REQUEST['id'];
		$qry = mysql_query($sql);
//		echo $sql;
      ...
      <tr>
        <td background="../decoup/model_fond.jpg"><table width="95%" border="0" align="center" cellpadding="1" cellspacing="1" class="text">
          ...
          <tr>
			...
			<form action="phototheque_modif.php?id=<? echo $_REQUEST['id']?>" method="post" onsubmit="MM_validateForm('nom','','R');return document.MM_returnValue" enctype="multipart/form-data">
			  <table width="100%" border="0" cellspacing="1" cellpadding="1">
                <tr>
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        <td align="left"><strong>Rubrique : </strong></td>
                        <td align="left"><select name="rubrique" class="text" id="rubrique">
                            <?
					$sqlr = "select * from photo_rubrique";
					$qryr = mysql_query($sqlr);
					while ($recr = mysql_fetch_array($qryr)) {
					?>
                            <option value="<? echo $recr['id_photo_rubrique']; ?>" <? if ($rec['idrubrique_photo'] == $recr['id_photo_rubrique']) echo "selected"; ?>><? echo $recr['nom_photo_rubrique']; ?></option>
                            <? }?>
                          </select>
                        </td>
                      </tr>
                      <tr>
                        ...
                        <td align="left"><input name="nom" type="text" class="text" id="nom" size="50"value="<? echo $rec['nom_photo']; ?>">
                        </td>
                      </tr>
                  </table></td>
                </tr>
                <tr>
                  <td><table width="100%" border="0" cellspacing="0" cellpadding="0">
                      <tr>
                        ...
                        <td align="left"><input name="fto" type="file" class="text" id="fto" size="20" />
                          <input name="foto" type="hidden" id="foto"  value="<? echo $rec['photo_photo'];?>" />
                          Choisir une autre image </td>
                      </tr>
                      <tr>
                        <td align="left">&nbsp;</td>
                        <td align="left"><img src="../images/phototheque/<? echo $rec['photo_photo']; ?>" width="150"/></td>
                      </tr>
                  </table></td>
                </tr>
                <tr>
