			...
			<table width="100%" border="0" cellspacing="1" cellpadding="1">
              <tr>
                <td><table width="100%" border="0" cellspacing="1" cellpadding="1">
                  <tr>
					<? if((strlen($rec['photo1_chapitre'])>0) || (strlen($rec['photo2_chapitre'])>0) || (strlen($rec['photo3_chapitre'])>0)) { ?>				
                    <td width="240" align="center" valign="top">
					<?
					if (strlen($rec['photo1_chapitre'])>0) {					
					$t1 = getimagesize("images/village/".$rec['photo1_chapitre']);
					?>
					<img src="images/village/<? echo $rec['photo1_chapitre']?>" width="200" onclick="MM_openBrWindow('admin/photozoom.php?fn=<? echo "images/village/".$rec['photo1_chapitre']?>','','width=<? echo $t1[0]; ?>,height=<? echo $t1[1]; ?>')">
					<? }?>
					 <br /><br /> 
					<?
					if (strlen($rec['photo2_chapitre'])>0) {
					$t2 = getimagesize("images/village/".$rec['photo2_chapitre']);
					?>
					<img src="images/village/<? echo $rec['photo2_chapitre']?>" width="200" onclick="MM_openBrWindow('admin/photozoom.php?fn=<? echo "images/village/".$rec['photo2_chapitre']?>','','width=<? echo $t2[0]; ?>,height=<? echo $t2[1]; ?>')">
					<? }?>
					<br /><br />
					<?
					if (strlen($rec['photo3_chapitre'])>0) {					
					$t3 = getimagesize("images/village/".$rec['photo3_chapitre']);
					?>
					<img src="images/village/<? echo $rec['photo3_chapitre']?>" width="200" onclick="MM_openBrWindow('admin/photozoom.php?fn=<? echo "images/village/".$rec['photo3_chapitre']?>','','width=<? echo $t3[0]; ?>,height=<? echo $t3[1]; ?>')">
					<? }?>
					</td>
					<? }?>
                    <td valign="top">
					<table width="100%" border="0" cellpadding="1" cellspacing="1" class="texttitre">
					<? if (strlen($_REQUEST['village']) > 0) { ?>
                      <tr>
                        <td align="center"><strong><? echo $_REQUEST['village']; ?></strong></td>
                      </tr>
					<? } ?>
					<? if (strlen($rec['nom_chapitre']) > 0) { ?>
                      <tr>
                        <td align="center"><? echo ucwords($rec['nom_chapitre']); ?></td>
                      </tr>
					<? } ?>
                    </table>
					<table width="100%" border="0" cellspacing="1" cellpadding="1">
                      <? if (strlen($rec['texte_chapitre']) > 0) { ?>
                      <tr>
                        <td align="left"><br />
                            <b>Contenu :</b><br />
                          <? echo $rec['texte_chapitre']; ?></td>
                      </tr>
                      <? } ?>
                    </table></td>
                  </tr>
...
<map name="Map" id="Map"><area shape="rect" coords="76,35,127,96" href="http://arpcv.free.fr" target="_blank" />
</map></body>
</html>