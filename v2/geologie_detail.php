<?
	...
	$sql = "select * from geo where id_geo = ".$_REQUEST['id'];
	$qry = mysql_query($sql) or die(mysql_error());
	$rec = mysql_fetch_assoc($qry);
	$nbrec = mysql_num_rows($qry);
	if ($nbrec == 0) header("location:geologie.php");
	mysql_close();
    ...
    <td width="780" align="center" valign="top"><table width="780" border="0" cellspacing="0" cellpadding="0">
      ...
      <tr>
        <td background="decoup/model_fond.jpg"><table width="95%" border="0" align="center" cellpadding="1" cellspacing="1" class="text">
          <tr>
            <td align="left"><img src="decoup/titre_geologie.jpg" width="207" height="26" /></td>
          </tr>
          <tr>
            <td><p align="right">
			...
			<? } else { ?>
			<a href="geologie.php" class="text">Retour</a>
			<? } ?>
			</p>
			<table width="100%" border="0" cellspacing="1" cellpadding="1">
              <tr>
                <td><table width="100%" border="0" cellspacing="1" cellpadding="1">
                  <tr>
					<? if((strlen($rec['photo1_geo'])>0) || (strlen($rec['photo2_geo'])>0) || (strlen($rec['photo3_geo'])>0)) { ?>				
                    <td width="240" align="center" valign="top">
					<?
					if (strlen($rec['photo1_geo'])>0) {					
					$t1 = getimagesize("images/geologie/".$rec['photo1_geo']);
					?>
					<img src="images/geologie/<? echo $rec['photo1_geo']?>" width="200" onclick="MM_openBrWindow('photozoom.php?fn=<? echo "images/geologie/".$rec['photo1_geo']?>','','width=<? echo $t1[0]; ?>,height=<? echo $t1[1]; ?>')">
					<? }?>
					 <br /><br /> 
					<?
					if (strlen($rec['photo2_geo'])>0) {
					$t2 = getimagesize("images/geologie/".$rec['photo2_geo']);
					?>
					<img src="images/geologie/<? echo $rec['photo2_geo']?>" width="200" onclick="MM_openBrWindow('photozoom.php?fn=<? echo "images/geologie/".$rec['photo2_geo']?>','','width=<? echo $t2[0]; ?>,height=<? echo $t2[1]; ?>')">
					<? }?>
					<br /><br />
					<?
					if (strlen($rec['photo3_geo'])>0) {					
					$t3 = getimagesize("images/geologie/".$rec['photo3_geo']);
					?>
					<img src="images/geologie/<? echo $rec['photo3_geo']?>" width="200" onclick="MM_openBrWindow('photozoom.php?fn=<? echo "images/geologie/".$rec['photo3_geo']?>','','width=<? echo $t3[0]; ?>,height=<? echo $t3[1]; ?>')">
					<? }?>
					</td>
					<? }?>
                    <td valign="top">
					<table width="100%" border="0" cellpadding="1" cellspacing="1" class="texttitre">
					<? if (strlen($rec['nom_geo']) > 0) { ?>
                      <tr>
                        <td align="center"><b><? echo ucwords($rec['nom_geo']); ?></b></td>
                      </tr>
					<? } ?>
                    </table>
					<table width="100%" border="0" cellspacing="1" cellpadding="1">
                      <? if (strlen($rec['texte_geo']) > 0) { ?>
                      <tr>
                        <td align="left"><br />
                            <b>Texte :</b><br />
                          <? echo $rec['texte_geo']; ?></td>
                      </tr>
                      <? } ?>
                    </table></td>
                  </tr>
...
<map name="Map" id="Map"><area shape="rect" coords="80,35,129,96" href="http://arpcv.free.fr" target="_blank" />
</map></body>
</html>